...
func ensureVMReady(hvCluster *KCluster, node ClusterNode) error {
	...
	if len(vmList) == 0 {
		...
		image := node.Image
		if image == "" {
			// Use default image based on OS type
...
func EnsureDKPClusterReady(config DKPClusterConfig, hvCluster *KCluster) (*KCluster, error) {
	...
	if setupNode == nil && len(config.ClusterDefinition.Workers) > 0 {
	...
	masterKeyPath := masterNode.Auth.SSHKey
	if masterKeyPath == "" {
